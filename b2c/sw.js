const CACHE='native-elaneeru-b2c-v10.26.0';
const SHELL=[
  './','./index.html','./login/index.html','./config.js','./manifest.webmanifest',
  './ui-v108.js','./ui-v110.js','./ui-v111.js','./ui-v112.js','./ui-v113.js','./ui-v125.js',
...
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==='navigate'){event.respondWith(networkFirst(request,navigationFallback(url)));return}const critical=/\/(?:config|ui-v108|ui-v110|ui-v111|ui-v112|ui-v113|ui-v125|separate-links|update-notifier|i18n-v102|i18n-v104)\.js$/;if(critical.test(url.pathname)||url.pathname.endsWith('/manifest.webmanifest')){event.respondWith(networkFirst(request));return}event.respondWith(cacheFirst(request))});
