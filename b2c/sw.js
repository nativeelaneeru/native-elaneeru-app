const CACHE='native-elaneeru-b2c-v10.36.0';
const SHELL=[
  './','./index.html','./login/index.html','./config.js','./manifest.webmanifest','./idb-v1.js','./order-sync-v1.js',
  './boot-recovery-v10360.js','./image-fallback-v918.js','./ui-v108.js','./ui-v110.js','./ui-v111.js','./ui-v112.js','./ui-v113.js','./ui-v125.js','./ui-v127.js',
...
self.addEventListener('install',event=>{
  event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(SHELL)).then(()=>{
    if(self.registration.active){return self.registration.showNotification('Native Elaneeru update ready',{body:'Latest reliability fixes are ready. Open the app to update.',tag:'native-elaneeru-update',icon:'./icons/icon-192.png',badge:'./icons/icon-192.png',data:{url:'./'}}).catch(()=>{});}
  }))
});
...
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==='navigate'){event.respondWith(networkFirst(request,navigationFallback(url)));return}const critical=/\/(?:config|boot-recovery-v10360|image-fallback-v918|idb-v1|order-sync-v1|ui-v108|ui-v110|ui-v111|ui-v112|ui-v113|ui-v125|ui-v127|separate-links|cart-visibility-v10320|customer-growth-v10330|referral-reward-v10340|payment-upi-v10350|update-notifier|i18n-v102|i18n-v104)\.js$/;if(critical.test(url.pathname)||url.pathname.endsWith('/manifest.webmanifest')){event.respondWith(networkFirst(request));return}event.respondWith(cacheFirst(request))});
