const CACHE='native-elaneeru-b2c-v10.32.0';
const SHELL=[
  ...
  './separate-links.js','./cart-visibility-v10320.js','./update-notifier.js','./i18n-v102.js','./i18n-v104.js',
...
self.addEventListener('install',event=>{
  event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(SHELL)).then(()=>{
    if(self.registration.active){return self.registration.showNotification('Native Elaneeru update ready',{body:'The latest Native Elaneeru fixes are ready. Open the app and tap Update now.',tag:'native-elaneeru-update',icon:'./icons/icon-192.png',badge:'./icons/icon-192.png',data:{url:'./'}}).catch(()=>{});}
  }))
});
...
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==='navigate'){event.respondWith(networkFirst(request,navigationFallback(url)));return}const critical=/\/(?:config|idb-v1|order-sync-v1|ui-v108|ui-v110|ui-v111|ui-v112|ui-v113|ui-v125|ui-v127|separate-links|cart-visibility-v10320|update-notifier|i18n-v102|i18n-v104)\.js$/;if(critical.test(url.pathname)||url.pathname.endsWith('/manifest.webmanifest')){event.respondWith(networkFirst(request));return}event.respondWith(cacheFirst(request))});
