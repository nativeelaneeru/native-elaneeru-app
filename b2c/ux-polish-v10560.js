(function(){
  if(window.NEL_UX_POLISH_V10560)return;
  window.NEL_UX_POLISH_V10560=true;

  function el(id){return document.getElementById(id)}
  function addStyle(){
    if(el('nelUx10560Style'))return;
    var s=document.createElement('style');
    s.id='nelUx10560Style';
    s.textContent=`
      :root{--nel-touch:44px}
  ...
  function simplifyHeader(){
    ...
    if(filters&&!el('nelQuickFilter')){
      ...
      quick.id='nelQuickFilter';quick.className='filter';quick.innerHTML='⚡ QUICK DELIVERY';
  ...
  function moveRewardsTeaser(){
    ...
    if(rail&&cash.parentNode!==home){rail.insertAdjacentElement('afterend',cash)}
  ...
  function positionGrowthCards(){
    ...
    if(heading){var h=heading.querySelector('h2');if(h)h.textContent='Account & plans'}
    if(form&&sub&&sub.parentNode===account)account.insertBefore(sub,form);
    if(form&&cash&&cash.parentNode===account)account.insertBefore(cash,form);
  }

  function observeGrowthCards(){
    var account=el('accountPage');if(!account)return;
    positionGrowthCards();
    var mo=new MutationObserver(function(){positionGrowthCards();labelFormControls()});
    mo.observe(account,{childList:true,subtree:false});
  }

  function boot(){
    addStyle();simplifyHeader();makeHeroUseful();moveRewardsTeaser();reorderBottomNav();labelFormControls();observeGrowthCards();
  }
