(function(){
  ...
  function renderLiveBanners(data){
    ...
    if(banners.length>1){var ix=0;setInterval(function(){if(!document.body.contains(rail))return;ix=(ix+1)%banners.length;var c=rail.children[ix];if(c&&c.scrollIntoView)c.scrollIntoView({behavior:'smooth',inline:'start',block:'nearest'})},4500)}
  }
  function renderReorder(data){
    ...
    host.innerHTML=ranked.map(function(x){var p=x.p,id=esc(p.productId),img=p.imageUrl?'<img src="'+esc(p.imageUrl)+'" alt="'+esc(p.productName)+'">':'🥥';return '<div class="fav"><div class="favVisual">'+img+'</div><div class="favBody"><b>'+esc(p.productName)+'</b><div class="meta">Most ordered · '+x.qty+' pcs</div><div class="price">₹'+Number(p.price||0).toLocaleString('en-IN')+' / '+esc(p.unit||'pc')+'</div><button class="add" onclick="add(\''+String(p.productId||'').replace(/'/g,"\\'")+'\')">ADD</button></div></div>'}).join('');
  }
  function renderOffers(data){
    var host=document.getElementById('offerProducts');if(!host)return;
    var products=[];try{products=(typeof S!=='undefined'&&S.cfg&&Array.isArray(S.cfg.products))?S.cfg.products:[]}catch(e){}
    if(!products.length)return;
    var bannerIds={};((data&&data.banners)||[]).forEach(function(b){if(b.productId)bannerIds[String(b.productId)]=1});
    var offerProducts=products.filter(function(p){var unit=Number(p.price||0),save1=Number(p.offerQty1||0)>0&&Number(p.offerPrice1||0)>0&&Number(p.offerPrice1)<unit*Number(p.offerQty1),save2=Number(p.offerQty2||0)>0&&Number(p.offerPrice2||0)>0&&Number(p.offerPrice2)<unit*Number(p.offerQty2);return save1||save2||bannerIds[String(p.productId)]});
    if(!offerProducts.length)offerProducts=products;
    try{host.innerHTML=offerProducts.map(function(p){var q=Number((typeof S!=='undefined'&&S.cart&&S.cart[p.productId])||0),offer=Number(p.offerQty2)?p.offerQty2+' for ₹'+Number(p.offerPrice2).toLocaleString('en-IN'):(Number(p.offerQty1)?p.offerQty1+' for ₹'+Number(p.offerPrice1).toLocaleString('en-IN'):'Fresh today'),control=q?'<div class="qty"><button onclick="change(\''+esc(p.productId)+'\',-1)">−</button><span>'+q+'</span><button onclick="change(\''+esc(p.productId)+'\',1)">+</button></div>':'<button class="add" onclick="add(\''+esc(p.productId)+'\')">ADD</button>',img=p.imageUrl?'<img src="'+esc(p.imageUrl)+'" alt="">':'🥥';return '<div class="product"><div class="visual">'+img+'</div><div class="pname">'+esc(p.productName)+'</div><div class="meta">'+esc(offer)+'</div><div class="price">₹'+Number(p.price||0).toLocaleString('en-IN')+' / '+esc(p.unit||'pc')+'</div>'+control+'</div>'}).join('')}catch(e){}
  }
  function hydrateProfile(r){
    if(!r||!r.exists||!r.profile)return;
    try{localStorage.setItem('nel_profile_v9',JSON.stringify(r.profile));localStorage.setItem('nel_b2c_profile',JSON.stringify(r.profile))}catch(e){}
    ['mobile','name','area','pincode','address'].forEach(function(k){var el=document.getElementById(k);if(el)el.value=r.profile[k]||''});
    try{if(typeof S!=='undefined'){S.lat=r.profile.latitude||'';S.lng=r.profile.longitude||''}}catch(e){}
    var g=document.getElementById('gpsText');if(g&&r.profile.latitude&&r.profile.longitude)g.textContent='GPS saved';
  }
  function postProcess(method,result,args){
    if(method==='getB2CAppDataV9'){renderLiveBanners(result);setTimeout(function(){renderOffers(result)},80)}
    if(method==='getCustomerDashboard'){renderReorder(result)}
    if(method==='customerLoginV95'){hydrateProfile(result)}
  }
  ...
  function hydrateSaved(){
    var m='';try{m=String(localStorage.getItem('nel_b2c_session_mobile')||'').replace(/\D/g,'').slice(-10)}catch(e){}
    if(!/^[6-9]\d{9}$/.test(m))return;
    bridgeRpc('customerLoginV95',[m]).then(function(r){hydrateProfile(r);if(r&&r.exists&&typeof window.loadCustomer==='function')return window.loadCustomer(false)}).catch(function(){});
  }
