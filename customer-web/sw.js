const CACHE='native-elaneeru-v10.6.0';
const SHELL=[
  './index.html','./login/index.html','./config.js','./manifest.webmanifest',
  './rpc-v106.js','./icons/native-elaneeru.svg','./icons/icon-192.png','./icons/icon-512.png',
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/config.js')||url.pathname.endsWith('/rpc-v106.js')){event.respondWith(fetchAndCache(event.request).catch(()=>caches.match(event.request)));return}
