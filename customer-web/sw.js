const CACHE='native-elaneeru-v10.16.0';
const SHELL=[
...
function enhanceHtml(response,isLogin){
  ...
  return response.text().then(text=>{
    ...
    if(!text.includes('ui-v112.js'))add.push('<script src="./ui-v112.js?v=10125"></script>');
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/config.js')||url.pathname.endsWith('/ui-v108.js')||url.pathname.endsWith('/ui-v110.js')||url.pathname.endsWith('/ui-v111.js')||url.pathname.endsWith('/ui-v112.js')){event.respondWith(fetchAndCache(event.request).catch(()=>caches.match(event.request)));return}
