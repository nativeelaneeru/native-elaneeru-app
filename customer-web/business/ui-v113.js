(function(){
  ...
  var CART_KEY='nel_b2b_cart_v113',restored=false;
  ...
  function addRetailSwitch(){
    ...
    var a=document.createElement('a');a.id='nel113Retail';a.className='nel113-retail';a.href='../';a.textContent='🥥 Retail';a.setAttribute('aria-label','Open Native Elaneeru retail store');
  ...
  function saveCart(){
    try{
      var out={};Object.keys(CART||{}).forEach(function(id){var q=Number(CART[id]&&CART[id].qty||0);if(q>0)out[id]=q});
      if(Object.keys(out).length)localStorage.setItem(CART_KEY,JSON.stringify(out));else localStorage.removeItem(CART_KEY);
    }catch(e){}
  }

  function restoreCart(){
    if(restored||!DATA||!Array.isArray(DATA.products))return;restored=true;
    try{
      var saved=JSON.parse(localStorage.getItem(CART_KEY)||'{}');
      Object.keys(saved).forEach(function(id){
        var p=DATA.products.find(function(x){return String(x.productId)===String(id)}),q=Math.floor(Number(saved[id]||0));if(!p||q<=0)return;
        var moq=Number(p.moq||1),step=Number(p.qtyStep||1);if(q<moq)q=moq;if(step>1&&q%step)q=Math.ceil(q/step)*step;
  ...
  function patchRender(){
    ...
    wrapped.__nel113=true;window.render=wrapped;try{render=wrapped}catch(e){};return true;
  }

  function patchAdd(){
    var base=window.add;if(typeof base!=='function'||base.__nel113)return typeof base==='function';
    var wrapped=function(){var r=base.apply(this,arguments);saveCart();return r};wrapped.__nel113=true;window.add=wrapped;try{add=wrapped}catch(e){};return true;
  }

  function patchRemove(){
    var base=window.removeItem;if(typeof base!=='function'||base.__nel113)return typeof base==='function';
    var wrapped=function(){var r=base.apply(this,arguments);saveCart();return r};wrapped.__nel113=true;window.removeItem=wrapped;try{removeItem=wrapped}catch(e){};return true;
  }
  ...
  function patchOrder(){
    ...
    var wrapped=async function(){
      ...
      try{return await base.apply(this,arguments)}finally{saveCart();var b=document.querySelector('#cartSheet .primary[onclick*="placeOrder"]');if(b){b.disabled=false;b.removeAttribute('aria-busy');if(old)b.textContent=old}}
    };
    wrapped.__nel113=true;window.placeOrder=wrapped;try{placeOrder=wrapped}catch(e){};return true;
  }

  function start(){
    style();addRetailSwitch();net();var tries=0;(function p(){var ok=patchRender()&&patchAdd()&&patchRemove()&&patchOrder();if(!ok&&++tries<120)setTimeout(p,100)})();
    setTimeout(function(){try{if(DATA){restored=false;restoreCart();if(typeof renderCart==='function')renderCart()}}catch(e){}},700);
  }
