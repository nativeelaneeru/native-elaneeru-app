...
function v930OfferDto_(r){return {bannerId:s_(r['Banner ID']),title:s_(r.Title),subtitle:s_(r.Subtitle),offerText:s_(r['Offer Text']),imageUrl:s_(r['Image URL']),audience:s_(r.Audience)||'B2C',productId:s_(r['Product ID']),startDate:v930SafeDate_(r['Start Date']),endDate:v930SafeDate_(r['End Date']),displayOrder:n_(r['Display Order']),status:s_(r.Status).toUpperCase()==='LIVE'?'LIVE':'PAUSED',redirectType:s_(r['Redirect Type']),redirectValue:s_(r['Redirect Value'])};}
