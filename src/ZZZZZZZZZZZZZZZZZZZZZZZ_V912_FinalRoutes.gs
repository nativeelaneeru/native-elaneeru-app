...
function doGetV912_(e){
  ...
  if(page==='Admin'){
    ...
    try{out.append(HtmlService.createTemplateFromFile('AdminBarcodeRealTestV960').evaluate().getContent());}catch(err){}
  }
  ...
  if(page==='VendorOnboardingV910'){
    ...
    try{out.append(HtmlService.createTemplateFromFile('VendorProspectsV965').evaluate().getContent());}catch(err){}
  }
  if(page==='Delivery'){
